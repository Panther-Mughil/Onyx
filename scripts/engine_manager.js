...
if (args.length < 2) {
    console.error("Usage: node engine_manager.js <action> <engine_id> [url_or_flags]");
    process.exit(1);
}
...
async function handleCompile() {
    ...
    const binaries = ['llama-server', 'llama-bench', 'ggml-rpc-server', 'llama-server.exe', 'llama-bench.exe'];
    ...
    for (const libDir of libDirs) {
        if (fs.existsSync(libDir)) {
            ...
            for (const file of files) {
                if (file.endsWith('.dylib') || file.endsWith('.so') || file.includes('.so.')) {
