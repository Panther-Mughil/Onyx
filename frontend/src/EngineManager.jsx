...
export default function EngineManager({ apiBase }) {
	...
	const checkActiveStatus = async () => {
		try {
			const res = await fetch(`${apiBase}/api/engines/status`);
			if (res.ok) {
				const data = await res.json();
				if (data.active_engine) {
					setActiveAction(data.active_engine);
					pollInstallStatus(data.active_engine);
				}
			}
		} catch (e) {
			console.error(e);
	...
	const stopEngine = async () => {
		try {
			const res = await fetch(`${apiBase}/api/engines/stop`, {
				method: "POST",
			});
			...
			console.error(e);
			alert(
				"Stop failed. Please ensure the backend is restarted with the latest changes.",
			);
		}
	};
	...
	const deleteEngine = async (id) => {
		...
		try {
			await fetch(`${apiBase}/api/engines/${encodeURIComponent(id)}`, {
				method: "DELETE",
			});
			fetchInstalled();
		} catch (e) {
	...
	// R2: macOS dependency banner
	const macDepsMissing = isMac
		? [!sysInfo?.has_xcode_clt, !sysInfo?.has_brew, !sysInfo?.has_cmake].filter(
				Boolean,
			)
		: [];
	const macBannerCommands = [];
	if (isMac) {
		if (!sysInfo?.has_xcode_clt)
			macBannerCommands.push("xcode-select --install");
		if (!sysInfo?.has_brew && sysInfo?.has_xcode_clt)
			macBannerCommands.push(
				'/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"',
			);
		if (!sysInfo?.has_cmake) macBannerCommands.push("brew install cmake");
	}

	const findAsset = (includes) =>
		releases.find(
			(a) =>
				includes.every((i) => a.name.includes(i)) &&
				a.name.endsWith(".zip") &&
				!a.name.includes("rpc") &&
				!a.name.includes("cudart"),
		);
	...
	if (isWin) {
		...
		const cudart13 = releases.find(
			(a) =>
				a.name.includes("cudart") &&
				a.name.includes("cuda-13") &&
				a.name.endsWith(".zip"),
		);
		const cudart12 = releases.find(
			(a) =>
				a.name.includes("cudart") &&
				a.name.includes("cuda-12") &&
				a.name.endsWith(".zip"),
		);
		if (hasNvidia) {
			...
			if (cuda13 && cpuAsset && cudart13) {
				options.push({
					id: "llama.cpp (CUDA 13)",
					label: "llama.cpp (CUDA 13)",
					asset: {
						browser_download_url: `${cpuAsset.browser_download_url},${cuda13.browser_download_url},${cudart13.browser_download_url}`,
					},
				});
			}
			if (cuda12 && cpuAsset && cudart12) {
				options.push({
					id: "llama.cpp (CUDA 12)",
					label: "llama.cpp (CUDA 12)",
					asset: {
						browser_download_url: `${cpuAsset.browser_download_url},${cuda12.browser_download_url},${cudart12.browser_download_url}`,
					},
				});
			}
		}
		...
		const isSilicon = macArch === "aarch64" || macArch === "arm64";
		const isIntel = macArch === "x86_64" || macArch === "x86";
		if (isSilicon) {
			options.push({
				id: "llama.cpp (Metal Silicon)",
			...
			>
				...
				{isLinux && (
					...
					>
						...
						>
							Before compiling, please run the following command in your
							terminal to ensure you have the required build tools:
						</p>
						<code
							style={{
				...
				{isMac && macDepsMissing.length > 0 && (
					...
					>
						...
						>
							Before compiling, please run the following command(s) in your
							terminal:
						</p>
						{macBannerCommands.map((cmd, i) => (
							<code
								key={i}
								style={{
									background: "rgba(0,0,0,0.5)",
						...
						return (
							...
							>
								...
								<div style={{ display: "flex", gap: "8px" }}>
									{isActive ? (
										<button
											className="engine-action-btn stop-btn"
											onClick={stopEngine}
										>
											Stop
										</button>
										...
										<button
											className="engine-action-btn compile-btn"
											onClick={() => compileEngine(opt.id, opt.compile)}
										>
											Compile
										</button>
									) : opt.asset ? (
										<button
											className="engine-action-btn compile-btn"
											onClick={() =>
												downloadEngine(opt.id, opt.asset.browser_download_url)
											}
										>
											Download
										</button>
